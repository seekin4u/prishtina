...
//	causeerrorheresoifixthis
	...
	var/obj/item/cyber/cyber_master = null
	...
	*/
	...
	var/rip_flags = 0 //Used in clothings
...
/obj/item/attack_hand(mob/user as mob)
	...
	if (hasorgans(user))
		...
		else if(user.hand == L_HAND)
			temp = H.organs_by_name["l_hand"]
		else
			user << "<span class='warning'>You can't pick up items by prosthesis slot</span>"
			return
...
/obj/item/proc/can_move(mob/user as mob)
	return canremove
...
/obj/item/proc/mob_can_equip(M as mob, slot, disable_warning = 0)
	...
	//Then, check special rules for the desired slot.
	switch(slot)
		...
		if(slot_s_store)
			...
			if(!H.wear_suit.allowed)
				...
				return 0
			if( !(istype(src, /obj/item/device/pda) || istype(src, /obj/item/weapon/pen) || istype(src, /obj/item/device/radio) || is_type_in_list(src, H.wear_suit.allowed)) )
				return 0
			if(src.w_class > 3.0)
				return 0
		if(slot_handcuffed)
	...
	//And last but not least, check for cyber
	if(istype(src, /obj/item/clothing))
		var/obj/item/clothing/W = src
		if(istype(W, /obj/item/clothing/suit/armor) || istype(W, /obj/item/clothing/suit/space))
			for(var/obj/item/cyber/C in H.cyber_modules)
				if(!C.is_bulk())
					continue
				if(!C.location)
					world.log << "Error 834275"
					continue
				var/obj/item/organ/external/O = C.location.get_external()
				if(O.body_part && W.body_parts_covered)
					H << "<span class='warning'>[C.name] preventing from wearing [W.name]</span>"
					return 0
	return 1
...
/obj/item/proc/rip_clothes(var/flag)
	if(!flag)
		return
	rip_flags |= flag
	update_icon()
