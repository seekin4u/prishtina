			...
			if(ishuman(imp_in))
				if (elevel == "Localized Limb")
					if(part) //For some reason, small_boom() didn't work. So have this bit of working copypaste.
						...
						sleep(25)
						var/obj/item/organ/external/ext_part = part.get_external()
						if (istype(ext_part,/obj/item/organ/external/chest) ||	\
							istype(ext_part,/obj/item/organ/external/groin) ||	\
							istype(ext_part,/obj/item/organ/external/head))
							ext_part.createwound(BRUISE, 60)	//mangle them instead
							explosion(get_turf(imp_in), -1, -1, 2, 3)
							qdel(src)
						else
							explosion(get_turf(imp_in), -1, -1, 2, 3)
							ext_part.droplimb(0,DROPLIMB_BLUNT)
							qdel(src)
