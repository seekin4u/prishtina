 ...
 **/
	proc/DivideOccupations()
		...
		var/spawnto = RUFORCE
		for(var/level = 1 to 3)
			var/list/rucandidates = FindSideOccupationCandidates(RUFORCE, level)
			var/list/encandidates = FindSideOccupationCandidates(ENFORCE, level)
...
/datum/controller/occupations/proc/is_side_locked(side)
	...
	if(side == ENFORCE)
		return !ticker.can_latejoin_enforce
	return 0
...
/datum/controller/occupations/proc/can_join_side(side)
	...
	if(!ticker)
		return 0
	if(side == RUFORCE)
		return (ruforce_count < enforce_count) && ticker.can_latejoin_ruforce
	if(side == ENFORCE)
		return (enforce_count < ruforce_count + 4) && ticker.can_latejoin_enforce
	if(side == CIVILIAN)
