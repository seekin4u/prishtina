/datum/hud/proc/human_hud(var/ui_style='icons/mob/screen1_White.dmi', var/ui_color = "#ffffff", var/ui_alpha = 255, var/mob/living/carbon/human/target)
	...
	if(hud_data.has_hands)
		...
		*/
		...
		inv_box.icon = ui_style
		inv_box.icon_state = "hand_inactive"
		if(mymob && mymob.hand == L_HAND)	//This being 1 means the left hand is in use
			inv_box.icon_state = "hand_active"
		...
		src.adding += using
		*/
	if(hud_data.has_prosthesis)
		inv_box = new /obj/screen/inventory()
		inv_box.name = "r_arm"
		inv_box.set_dir(WEST)
		inv_box.icon = ui_style
		inv_box.icon_state = "blank"
		if(istype(mymob, /mob/living/carbon/human))
			var/mob/living/carbon/human/H = mymob
			if(H.l_arm)
				inv_box.icon_state = "arm_inactive"
		if(mymob && !mymob.hand == R_ARM)
			inv_box.icon_state = "arm_active"
		inv_box.screen_loc = ui_rarm
		inv_box.slot_id = slot_r_arm
		inv_box.layer = 19
		inv_box.color = ui_color
		inv_box.alpha = ui_alpha
		src.r_arm_hud_object = inv_box
		src.adding += inv_box
		...
		if(istype(mymob, /mob/living/carbon/human))
			...
			if(H.l_arm)
				inv_box.icon_state = "arm_inactive"
		if(mymob && mymob.hand == L_ARM)	//This being 1 means the left hand is in use
			inv_box.icon_state = "arm_active"
		inv_box.screen_loc = ui_larm
		inv_box.slot_id = slot_l_arm
		inv_box.layer = 19
		inv_box.color = ui_color
		inv_box.alpha = ui_alpha
		src.l_arm_hud_object = inv_box
		src.adding += inv_box
	...
	if(hud_data.has_internals)
		...
		mymob.internals.icon_state = "internal_off"
