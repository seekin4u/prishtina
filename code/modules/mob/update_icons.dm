...
/mob/proc/update_inv_l_arm()
	return

/mob/proc/update_inv_r_arm()
	return

/mob/proc/update_inv_hand(var/hand, var/update_icons = 1)
	if(hand == R_HAND)
		update_inv_r_hand(update_icons)
		return
	if(hand == L_HAND)
		update_inv_l_hand(update_icons)
		return
	if(hand == R_ARM)
		update_inv_r_arm(update_icons)
		return
	if(hand == L_ARM)
		update_inv_l_arm(update_icons)
		return
