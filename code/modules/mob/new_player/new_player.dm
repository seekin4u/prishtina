...
/mob/new_player
	...
	Topic(href, href_list[])
		...
		if(href_list["observe"])
			if(!ticker || ticker.current_state != GAME_STATE_PLAYING)
				usr << "\red You can't go observe while game is not started or alreadt finished."
				return 0

			if(alert(src,"Are you sure you wish to observe? You will have to wait s0 minutes before being able to respawn!","Player Setup","Yes","No") == "Yes")
				if(!client)	return 1
	...
	proc/AttemptLateSpawn(rank,var/spawning_at)
		...
		var/datum/job/job = job_master.GetJob(rank)
		if(!job)
			src << "\red Something goes wrong in spawn proc"
			return 0
		...
		if(turfs && turfs.len > 0)
			character.loc = pick(turfs)
		else
			character << "\red Oh god. Bug with spawning! We are doomed!"
		/*
		var/datum/spawnpoint/S = spawntypes[job.spawn_location]
		...
		if(S && istype(S))
			...
			character.loc = pick(latejoin)
			character << "There is bug in spawn. Your job is [job.title]. Please report to admin."
			//join_message = "has arrived on the station"
		*/
