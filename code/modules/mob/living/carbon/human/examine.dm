/mob/living/carbon/human/examine(mob/user)
	...
	if(l_arm)
		if(l_arm.blood_DNA)
			msg += "<span class='warning'>[t_He] [t_is] wielding \icon[l_arm] [l_arm.gender==PLURAL?"some":"a"] [(l_arm.blood_color != "#030303") ? "blood" : "oil"]-stained [l_arm.name] in [t_his] left arm!</span>\n"
		else
			msg += "[t_He] [t_is] wielding \icon[l_arm] \a [l_arm] in [t_his] left arm.\n"

	if(r_arm)
		if(r_arm.blood_DNA)
			msg += "<span class='warning'>[t_He] [t_is] wielding \icon[r_arm] [r_arm.gender==PLURAL?"some":"a"] [(r_arm.blood_color != "#030303") ? "blood" : "oil"]-stained [r_arm.name] in [t_his] right arm!</span>\n"
		else
			msg += "[t_He] [t_is] wielding \icon[r_arm] \a [r_arm] in [t_his] right arm.\n"
