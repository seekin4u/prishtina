...
/mob/living/carbon/human
	...
	get_breath_from_cyber(var/volume_needed=BREATH_VOLUME)
		if(cyber_breath_source)
			var/datum/gas_mixture/breath = null
			breath = cyber_breath_source.handle_breathe()
			if(breath)
				if(internals)
					internals.icon_state = "internal_cyber"
				return breath
		return null
				...
				if(!found_welder && istype(back, /obj/item/weapon/rig))
					...
					if(O.helmet && O.helmet == head && (O.helmet.body_parts_covered & EYES))
						if((O.offline && O.offline_vision_restriction == 1) || (!O.offline && O.vision_restriction == 1))
							found_welder = 1
				if(!found_welder && istype(head, /obj/item/clothing/mask/gas))
					found_welder = 1
				if(found_welder)
