		...
		if(!lying)
			...
			if(istype(T) && T.stepsound)
				if(istype(src, /mob/living/carbon/human))
					...
					if(H.shoes)
						playsound(src, T.stepsound, 10, 0, 3, 1)
...
/mob/living/carbon/proc/swap_hand()
	...
	if(item_in_hand) //this segment checks if the item in your hand is twohanded.
		if(istype(item_in_hand,/obj/item/weapon/material/twohanded))
			if(item_in_hand:wielded == 1)
				...
				return
		if(istype(item_in_hand, /obj/item/weapon/gun))
			var/obj/item/weapon/gun/G = item_in_hand
			if(G.wielded)
				G.unwield(src)

	//##TODO CODE reduce image change count
	if(hand == R_HAND)
		hand = L_HAND
	else if(hand == L_HAND)
		hand = R_HAND
	...
	if(hud_used.l_hand_hud_object && hud_used.r_hand_hud_object)
		if(hand)	//This being 1 means the left hand is in use
			...
			//hud_used.l_arm_hud_object.icon_state = "hand_inactive"
			//hud_used.r_arm_hud_object.icon_state = "hand_inactive"
		else
