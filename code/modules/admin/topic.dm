...
//		if(!check_rights(R_BAN))	return
		...
		for(var/datum/job/job in job_master.occupations)
			if(last_side != job.department_flag)
				if(last_side != null)
					dat += "<br>"
				last_side = job.department_flag
				dat += "<a href='?src=\ref[src];jobban3=[get_side_name(last_side)];jobban4=\ref[M]'><b>[get_side_name(last_side)]</b></a><br>"

			if(jobban_isbanned(M, job.title))
				dat += "<a href='?src=\ref[src];jobban3=[job.title];jobban4=\ref[M]'><font color='red'>[job.title]</font></a><br>"
			else
				dat += "<a href='?src=\ref[src];jobban3=[job.title];jobban4=\ref[M]'>[job.title]</a><br>"

		usr << browse(dat, "window=jobban2;size=300x800")
		/*
		var/dat = ""
...
//Regular jobs
	...
	//JOBBAN'S INNARDS
		...
		switch(href_list["jobban3"])
			...
			if("RuForce")
				for(var/datum/job/job in job_master.occupations)
					if(job.department_flag == RUFORCE)
						joblist += job
			if("EnForce")
				for(var/datum/job/job in job_master.occupations)
