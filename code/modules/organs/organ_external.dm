...
/obj/item/organ/external/take_damage(brute, burn, sharp, edge, used_weapon = null, list/forbidden_limbs = list())
	...
	//first, cyber modules take damage
	//they are like armor huh
	if(cyber_modules.len)
		if(brute > 3)
			var/obj/item/cyber/C = pick(cyber_modules)
			var/dmg
			if(prob(C.ext_hit_prob / 2)) //absorb all damage we can
				dmg = C.can_absorb_damage(brute)
			else if(prob(C.ext_hit_prob))
				dmg = C.can_absorb_damage(brute / 2)
			if(dmg)
				C.take_damage(dmg)
				brute -= dmg
	...
	if(internal_organs && (brute_dam >= max_damage || (((sharp && brute >= 5) || brute >= 10) && prob(10))))
...
/obj/item/organ/external/get_external()
	return src
