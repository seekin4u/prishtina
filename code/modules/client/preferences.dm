...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	//dat += "<b>Home city</b>: <a href='byond://?src=\ref[user];preference=home_system;task=input'>[home_system]</a><br/>"
			...
			switch(href_list["preference"])
				...
				if("lobby_music")
					...
					if(toggles & SOUND_LOBBY)
						user << sound(ticker.login_music, repeat = 1, wait = 0, volume = 60, channel = 1)
					else
						user << sound(null, repeat = 1, wait = 0, volume = 10, channel = 1)
